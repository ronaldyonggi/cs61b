...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
        ...
        return presentValue*Math.pow(multiplier, years);
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        /**
         * inflationMultiplier is the multiplier for inflation, already taking into account
         * the power by years. For example, 0.97^4.
         */
        double inflationMultiplier = Math.pow(1 - (inflationRate / 100), numYears(targetYear));
        return futureValue(presentValue, rate, targetYear) * inflationMultiplier;
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int years = numYears(targetYear);
        double total = 0;
        double rateMultiplier = 1 + (rate / 100);
        /**
         * Use a for loop to sum the PERYEAR each year. For every loop, the year decreases.
         */
        for (int i = years; i >= 0; i--) {
            total += perYear * Math.pow(rateMultiplier, i);
        }
        return total;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        int years = numYears(targetYear);
        double beforeInflation = totalSavings(perYear, targetYear, rate);
        double inflationMultiplier = Math.pow(1 - (inflationRate/100), years);
        return beforeInflation*inflationMultiplier;
